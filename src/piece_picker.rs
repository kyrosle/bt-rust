...
pub struct PiecePicker {
    ...
    ///
    /// The bitfield is pre-allocated to the number of pieces
    /// in the torrent and each field that we have is set to
    /// true.
    own_pieces: Bitfield,
    /// We collect metadata about pieces in the torrent
    /// swarm in this vector.
    ///
    /// The vector is pre-allocated to the number of pieces
    /// in the torrent.
    pieces: Vec<Piece>,
    /// A cache for the number of pieces we haven't received
    /// yet (but may have picked).
    missing_count: usize,
...
pub struct Piece {
    ...
    ///
    /// This is to prevent picking the same piece we are already downloading
    /// in the scenario in which we want to pick a new piece before the already
...
impl PiecePicker {
    ...
    /// Returns true if all pieces have been picked (whether pending or received).
    ...
    /// Registers the availability of a peer's pieces and whether we're
    ...
    pub fn register_peer_pieces(
        &mut self,
        pieces: &Bitfield,
    ) -> bool {
        log::trace!(
            "Registering piece availability: {}",
            pieces
        );
    ...
    pub fn register_peer_piece(
        &mut self,
        index: PieceIndex,
    ) -> bool {
        log::trace!(
            "Registering newly available piece {}",
            index
        );
    ...
    pub fn received_piece(
        &mut self,
        index: PieceIndex,
    ) {
        log::trace!(
            "Registering received piece {}",
            index
        );
        ...
        *have_piece = true;
        ...
        // against it anyway in case there are changes in order parts of the
        // code.
        ...
        if !piece.is_pending {
            ...
            // won't be downloading it anymore, later we may re-download a piece in
            // which case not resetting the flag would cause us to never pick
            // pick the piece again)
...
mod tests {
    ...
    fn should_pick_all_pieces() {
        ...
        let mut piece_picker =
            PiecePicker::empty(piece_count);
        let available_pieces =
            Bitfield::repeat(true, piece_count);
        piece_picker
            .register_peer_pieces(&available_pieces);

        // save picked pieces
        let mut picked =
            HashSet::with_capacity(piece_count);
    ...
    fn should_mark_piece_as_received() {
        ...
        for _ in 0..piece_count - owned_pieces.len() {
            let pick =
                piece_picker.pick_piece().unwrap();
            // assert that it's not a piece we already have
            assert!(owned_pieces
                .iter()
                .all(|owned| *owned != pick));
        }
    }
    ...
    fn should_count_free_pieces() {
        ...
        piece_picker.register_peer_pieces(
            &Bitfield::repeat(true, piece_count),
        );

        assert_eq!(
            piece_picker.free_count,
            piece_count
        );
