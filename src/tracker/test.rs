...
mod tests {
    ...
    fn should_parse_full_peer_list() {
        ...
        let decoded: PeersResponse =
            serde_bencode::from_str(&encoded).expect(
                "cannot decode bencode list of peers",
            );
    ...
    async fn should_return_peers_on_announce() {
        ...
        peer_id
            .copy_from_slice(peer_id_str.as_bytes());
        ...
        let expected_resp = Response {
            ...
            min_interval: Some(Duration::from_secs(
                10,
            )),
            seeder_count: Some(5),
