...
use crate::{
    blockinfo::BlockInfo,
    peer::codec::message::MessageId, Bitfield,
};
...
impl Decoder for PeerCodec {
    ...
    fn decode(
        ...
        if buf.remaining() >= 4 + msg_len {
            ...
            // (all other message types have at least one more field, the message id)
            if msg_len == 0 {
        ...
        let msg = match msg_id {
            ...
            MessageId::Bitfield => {
                ...
                Message::Bitfield(Bitfield::from_vec(
                    bitfield,
                ))
            }
            MessageId::Request => {
...
mod tests {
    ...
    fn test_message_stream() {
        ...
        let decoded_handshake = HandshakeCodec
            .decode(&mut read_buf)
            .unwrap();
        assert_eq!(decoded_handshake, Some(handshake));
        for (msg, _) in &msgs {
            let decoded_msg = PeerCodec
                .decode(&mut read_buf)
                .unwrap();
    ...
    // `test_test_message_stream`, by progressively loading up the codec's read
    ...
    fn test_invalid_handshake_decoding() {
        ...
        let result = HandshakeCodec
            .decode(&mut invalid_encoded);
    ...
    fn make_handshake() -> (Handshake, Bytes) {
        ...
        // TODO: consider using hard coded byte array for expected value rather
    ...
    fn make_request() -> (Message, Bytes) {
        ...
        let encoded =
            make_block_info_encoded_msg_payload(
                MessageId::Request,
                piece_index,
                offset,
                len,
