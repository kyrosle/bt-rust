...
pub struct SessionState {
  ...
  /// If peer is interested, they mean to download pieces that we have.
  pub is_peer_interested: bool,
}

impl Default for SessionState {
  /// By default, both sides of the connection start off as choked and
...
/// At any given time, a connection with a peer is in one the below states.
...
pub enum ConnectionState {
  ...
  /// occurs after the handshake and not later. It is set once the handshakes
  /// are exchanged and changed as soon as we receive the bitfield or the fist
  /// message that is not a bitfield. Any subsequent bitfield messages
...
/// Holds and provides facilities to modify the state of
/// a peer session.
#[derive(Default)]
pub struct SessionContext {
  ...
  /// If this flag is true, the peer session needs to send
  /// a state update message to torrent.
  ///
  ...
  ///
  /// To keep up with the transport layer's slow start algorithm
  /// (which unlike its name, exponentially increases window size)
  /// a `peer_session` starts out in slow start as well, wherein
  /// the target request queue size is increased by one every
  /// time one of our requests got served, doubling the queue size
  ...
  /// incur synchronization costs on the hot path.
  pub in_endgame: bool,

  /// The target request queue size is the number of block
  /// requests we keep outstanding to fully saturate the link.
  ///
  /// Each peer session needs to maintain an
  /// "optimal request queue size" value
  /// (approximately the bandwidth-delay product),
  /// which is teh number of block requests it
  /// keeps out outstanding to fully saturate the link.
  ///
  /// This value is derived by collecting a running average
  /// of the downloaded  bytes per second, as well as the
  /// average request latency, to arrive at the bandwidth-delay
  /// product B x D. This value recalculated every time we
  /// receive a block, in order to always keep the link fully
  /// saturated.
  ///
  /// ```text
...
impl SessionContext {
  /// When we check whether to exist slow start mode we want to allow for
  /// some error margin. This is because there may be "micro-fluctuations"
  /// in the download rate but per second but over a longer time the download
  /// rate may still be increasing significantly.
  const SLOW_START_ERROR_MARGINS: u64 = 10000;

  /// The target request queue size is set to this value once we are able to
  /// start downloading.
  ...
  ///
  /// We leave slow start if the download rate has not increased significantly
  ...
  fn update_target_request_queue_len(&mut self) {
    if let Some(target_request_queue_len) = &mut self.target_request_queue_len {
      ...
      if !self.in_slow_start {
        ...
        *target_request_queue_len = ((download_rate + (BLOCK_LEN - 1) as u64)
