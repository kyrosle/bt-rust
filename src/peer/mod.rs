...
impl PeerSession {
    ...
    async fn start(
            ...
            socket.next().await
        {
            ...
            if let Err(e) = self.run(socket).await {
                ...
                self.torrent.alert_tx.send(
                    Alert::Error(Error::Peer {
                        id: self.torrent.id,
                        addr: self.peer.addr,
                        error: e,
                    }),
    ...
    async fn make_requests(
            ...
            .values_mut()
        {
            ...
            let outgoing_request_count = requests
                .len()
                + self.outgoing_requests.len();
    ...
    async fn handle_block_msg(
        ...
        if prev_status == BlockStatus::Received {
            ...
            self.ctx
                .update_download_stats(block_info.len);
