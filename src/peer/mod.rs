//! This module defines the implementation of the BitTorrent peer write protocol.
//!
...
use crate::{
  ...
  peer::{
    codec::{
      handshake::{
        Handshake, HandshakeCodec, PROTOCOL_STRING,
      },
...
/// After this timeout if the peers haven't become interested in each other,
...
/// it is responsible for exchanging the BitTorrent messages that
/// drive a download.
/// It only concerns itself with the network aspect of things:
/// disk IO, for example, is delegated to the [disk task](crate::disk::Disk).
///
...
/// # Important
///
/// For now only the BitTorrent v1 specification is implemented, without any extensions.
pub struct PeerSession {
  ...
  ///
  /// A copy of this is kept within peer session as disk block reads are communicated
  /// back to session directly via its command port. For this, we need to pass a
  /// copy of the sender with each block read to the disk task.
  cmd_tx: Sender,
  ...
  /// Most of the session's information and state is stored here,
  /// i.e. it's the "context" of the session.
  ctx: SessionContext,
  ...
  ///
  /// This structure's main purpose is for the timeout logic:
  /// if the peer times out, the currently pending blocks requested from
  /// peer are freed for other peer sessions to download. The block requests
  /// are not actually cancelled, since the block will usually arrive after
  /// some time, but we want to give other sessions a chance to download it faster.
  /// Therefore some blocks may arrive twice, in which case we drop the second arrival:
  ...
  /// the blocks the timed out peer would eventually send would be highly
  /// wasteful. In practice even with fast seeds occasional timeouts occur, at
  /// which point we may have hundreds if not thousands of pending blocks in the
  /// pipeline.
  ///
  ...
  /// actually be re-downloaded by other peers, so only a fraction of these
  /// will be wasted. Thus this method avoids bandwidth wast and cuts down
  ...
  /// be served.
  ///
  /// Note that if a reused for a piece's block is in this queue, there must
  /// be a corresponding entry for the piece download in `download`
  ///
  /// TODO: Can we store this information in just PieceDownload so that we don't
  /// have to enforce this invariant (keeping in mind that later PieceDownload will
  /// be shared among PeerSession)>
...
pub struct PeerInfo {
  ...
  ///
  /// This is equivalent to `self.pieces.count_ones()` and
  /// is updated every time the peer sends us an announcement
  /// of a new piece.
  pub piece_count: usize,
}
...
impl PeerSession {
  ...
  pub async fn start_outbound(
    ...
    self.ctx.set_connection_state(
      ConnectionState::Connecting,
    );
    let socket =
      TcpStream::connect(self.peer.addr).await?;
  ...
  async fn start(
    ...
    if let Some(peer_handshake) = socket.next().await {
      ...
      // message codec and save the socket in self
      // (note that we need to keep the buffer from the original codec as
      // it may contain bytes of any potential message the peer may have
      // sent after the handshake)
      let old_parts = socket.into_parts();
      let mut new_parts =
        FramedParts::new(old_parts.io, PeerCodec);
      ...
      self.torrent.cmd_tx.send(
        torrent::Command::PeerConnected {
          addr: self.peer.addr,
      ...
      if let Err(e) = self.run(socket).await {
        ...
        self.torrent.cmd_tx.send(
          torrent::Command::PeerState {
            addr: self.peer.addr,
            info: self.session_info(),
          },
        )?;
        self.torrent.alert_tx.send(Alert::Error(
          Error::Peer {
            id: self.torrent.id,
            addr: self.peer.addr,
            error: e,
    ...
    // session exited as a result of a clean shutdown or an error,
  ...
  async fn run(
    ...
    // available, send a bitfield message.
    {
      let piece_picker_guard =
        self.torrent.piece_picker.read().await;
      ...
      if own_pieces.any() {
        ...
        sink
          .send(Message::Bitfield(own_pieces.clone()))
          .await?;
  ...
  ///
  /// This is when we update statistics and report them to torrent
  ...
  async fn tick(
    ...
    // reset requests if we have pending requests and more time has elapsed
    ...
    let prev_queue_len =
      self.ctx.target_request_queue_len;
    self.ctx.tick();
    if let (
      Some(prev_queue_len),
      Some(curr_queue_len),
  ...
  async fn check_request_timeout(
      ...
      self.ctx.last_outgoing_request_time
    {
      ...
      if elapsed_since_last_request > request_timeout {
        ...
        // (since we can only request a single block now).
        // Start by freeing up the blocks in their piece download.
  ...
  /// Marks requests blocks as free in their respective downloads so that
  ...
  async fn free_pending_blocks(&mut self) {
    ...
    for block in self.outgoing_requests.drain() {
      ...
      if let Some(download) =
        downloads_guard.get(&block.piece_index)
      {
        log::debug!(
            target: &self.ctx.log_target,
  ...
  async fn handle_bitfield_msg(
    ...
    // According to the spec if the remainder contains any non-zero bits,
    // we need to abort the connection. Not sure if this is too strict,
    // there doesn't seem much harm in it so we skip the check.
    bitfield
      .resize(self.torrent.storage.piece_count, false);
    ...
    self.peer.piece_count =
      self.peer.pieces.count_ones();

    if self.peer.piece_count
  ...
  async fn handle_msg(
    ...
    match msg {
      ...
      Message::Unchoke => {
        if self.ctx.state.is_choked {
          ...
          if self.ctx.state.is_interested {
            ...
            // not that we are allowed to request blocks, start the
      ...
      Message::Have { piece_index } => {
        self
          .handle_have_msg(sink, piece_index)
          .await?;
      }
  ...
  async fn make_requests(
    ...
    // TODO: optimize this by using the preallocated hash-set in self
    let mut requests = Vec::new();
    let target_request_queue_len = self
      .ctx
      .target_request_queue_len
      .unwrap_or_default();

    // If we have active downloads, prefer to continue those.
    // This will result in less in-progress pieces.
    for download in
      self.torrent.downloads.write().await.values_mut()
    {
      ...
      // our outgoing request queue shouldn't exceed the allowed
      // request queue size.
      if outgoing_request_count
        >= target_request_queue_len
      {
        break;
      }

      let to_request_count = target_request_queue_len
        - outgoing_request_count;

      let mut download_write_guard =
        download.write().await;
  ...
  async fn handle_block_msg(
    ...
    if prev_status == BlockStatus::Received {
      ...
      self.torrent.disk_tx.send(
        disk::Command::WriteBlock {
          id: self.torrent.id,
          block_info,
          data,
        },
  ...
  /// [`Self::run`]. This is when the block is actually sent to peer,
  /// if by the request is not cancelled by then.
  ...
  async fn send_block(
    ...
    let was_present =
      self.incoming_requests.remove(&info);
  ...
  async fn update_interest(
    ...
    } else if self.ctx.state.is_interested
      && !is_interested
    {
      log::info!(
          target: &self.ctx.log_target,
  ...
  fn validate_block_info(
    ...
    let piece_len =
      self.torrent.storage.piece_len(info.piece_index);
    if info.len > 0
  ...
  /// When the torrent completes a new piece, peer sessions are notified of it.
  ///
