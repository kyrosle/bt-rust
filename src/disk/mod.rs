...
use crate::{
    blockinfo::BlockInfo, engine, error::*, peer,
...
pub fn spawn(
    ...
    let join_handle =
        task::spawn(async move { disk.start().await });
...
impl Disk {
            ...
            match cmd {
                Command::NewTorrent {
                    ...
                    match torrent_res {
                        Ok(torrent) => {
                            log::info!(
                                "Torrent {} successfully allocated", 
                                id
                            );
    ...
    async fn read_block(
        ...
        log::trace!(
            "Reading torrent {} block {} from disk",
            id,
            block_info
        );
...
mod tests {
    ...
    use crate::{
        ...
        storage_info::FileInfo, BLOCK_LEN,
    };
    ...
    async fn should_write_all_pieces() {
        ...
        rx.recv()
            .await
            .expect("cannot allocate torrent");
        ...
        for (index, piece) in pieces.iter().enumerate()
        {
            ...
            if let Some(
                torrent::Command::PieceCompletion(Ok(
                    piece,
                )),
        ...
        .expect(
            "cannot clean up disk test torrent file",
        );
    }
    ...
    async fn should_reject_writing_invalid_piece() {
        ...
        let index = 0;
        let invalid_piece: Vec<_> = pieces[index]
            .iter()
            .map(|b| b.saturating_add(5))
            .collect();
        for_each_block(
            index,
            invalid_piece.len() as u32,
            |block| {
                let block_end =
                    block.offset + block.len;
                let data = &invalid_piece[block.offset
                    as usize
                    ..block_end as usize];
                debug_assert_eq!(
                    data.len(),
        ...
        ) = torrent_rx.recv().await
        {
            ...
            panic!(
                "piece could not be written to disk"
            );
        }
    }
    ...
    async fn should_read_piece_blocks() {
        ...
        for _ in 0..block_count {
            ...
            // wait for result
            if let Some(peer::Command::Block(block)) =
                rx.recv().await
            {
    ...
    fn for_each_block(
        ...
        for _ in 0..block_count {
            // when calculating the block length we need to consider that the
            // last block may be smaller than the rest
            let block_len = (piece_len - block_offset)
                .min(BLOCK_LEN);
    ...
    impl Env {
        ...
        fn new(test_name: &str) -> Self {
            ...
            let mut piece_hashes =
                Vec::with_capacity(pieces.len() * 20);
            ...
            let download_len = pieces.iter().fold(
                0,
                |mut len, piece| {
