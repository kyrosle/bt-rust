    ...
    while let Some(cmd) = self.cmd_rx.recv().await {
      match cmd {
        ...
        Command::WriteBlock {
          ...
          self
            .write_block(id, block_info, data)
            .await?
        }
        Command::ReadBlock {
          id,
          ...
          self
            .read_block(id, block_info, result_tx)
            .await?
        }
        Command::Shutdown => {
...
mod tests {
  ...
  use crate::{
    blockinfo::block_count, storage_info::FileInfo,
    BLOCK_LEN,
  };
  ...
  async fn should_write_all_pieces() {
    ...
    for (index, piece) in pieces.iter().enumerate() {
      ...
      if let Some(torrent::Command::PieceCompletion(
        Ok(piece),
  ...
  async fn should_reject_writing_invalid_piece() {
    ...
    for_each_block(
      ...
      |block| {
        let block_end = block.offset + block.len;
        let data = &invalid_piece
          [block.offset as usize..block_end as usize];
        debug_assert_eq!(
          data.len(),
          block.len as usize
        );
  ...
  impl Env {
    ...
    fn new(test_name: &str) -> Self {
      ...
      let download_rel_path = PathBuf::from(format!(
      ...
      {
        let download_path =
          download_dir.join(&download_rel_path);
      ...
      let download_len =
        pieces.iter().fold(0, |mut len, piece| {
