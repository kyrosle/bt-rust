...
impl TorrentFile {
        ...
        while !iovecs.as_slice().is_empty() {
            ...
            let write_count = self
                ...
                .seek_write(
                    ...
                    file_slice.offset + total_write_count,
                )
                .map_err(|e| {
    ...
    pub fn read<'a>(
        &self,
        file_slice: FileSlice,
        mut iovecs: &'a mut [IoVec],
    ) -> Result<&'a mut [IoVec], ReadError> {
        ...
        for iovec in iovecs.iter_mut() {
            let mut data = vec![0u8; iovec.len()];
            let read_count = self
                .handle
            ...
            iovec.modify(&data).map_err(|e| {
                log::warn!(
                    "File {:?} read align error: expected: {}, actually: {}",
                    self.info.path,
                    iovec.len(),
                    e.len()
                );
                ReadError::InconsistentLength
            })?;
