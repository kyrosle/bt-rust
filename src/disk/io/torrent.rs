...
struct ThreadContext {
    ...
    /// tokio redis example recommends using sync mutexes if holding them across await
...
impl Torrent {
    ...
    pub fn new(
        ...
        let files = if info.files.len() == 1 {
            ...
            for file in info.files.iter() {
                ...
                // download root not exists

                debug_assert!(!path.exists());
                debug_assert!(path.is_absolute());

                // get the parent of the file path: if there is one (i.e.
                // this is not a flie in the torrent root), and doesn't
                // exist, crate it.
    ...
    pub fn write_block(
        ...
        let piece = self
            ...
            .get_mut(&piece_index)
            .expect(
                "Newly inserted piece not present",
            );

        piece.enqueue_block(info.offset, data);

        // if the piece has all its blocks,
        // it means we can hash it and save it to disk
        // and clear its write buffer.
        if piece.is_complete() {
            ...
            let torrent_piece_offset = self
                .info
                .torrent_piece_offset(piece_index);
    ...
    fn start_new_piece(
        ...
        let hash_slice = &self.piece_hashes
            [hash_pos..hash_pos + 20];
    ...
    pub fn read_block(
            ...
            .get(&piece_index)
        {
            ...
            result_tx.send(Command::Block(
                Block::new(block_info, block),
            ))?;
