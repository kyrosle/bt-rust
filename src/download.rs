...
impl PieceDownload {
    /// Crates a new piece download instance for the given piece.
    ...
    pub fn pick_blocks(
            ...
            self.blocks.iter_mut().enumerate()
        {
            ...
            if *block == BlockStatus::Free {
                ...
                *block = BlockStatus::Requested;
                ...
                && *block == BlockStatus::Requested
            {
                // in endgame it's to pick blocks already requested but
                ...
                // TODO: could be optimized by checking if peer is present
        ...
        if picked > 0 {
            ...
            log::trace!(
                "Cannot pick any blocks in piece {}",
                self.index
            );
        }
    }
...
mod tests {
    ...
    fn should_pick_all_blocks_one_by_one() {
        ...
        // save picked blocks
        let mut picked =
            HashSet::with_capacity(block_count);
    ...
    fn should_pick_all_blocks_in_new_download() {
        ...
        let mut download =
            PieceDownload::new(piece_index, piece_len);
