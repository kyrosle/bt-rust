...
pub struct PieceStats {
    ...
    pub pending: usize,
    /// The number of pieces that the torrent is has downloaded.
    pub complete: usize,
    /// The pieces that were completed since the last tick.
    ///
    /// By default this information is not sent, as it has some overhead.
    /// It needs to be turned on in the torrent's [configuration]
...
impl PieceStats {
    ...
    /// Returns whether the torrent is in endgame mode
    /// (about to finish download).
    pub fn is_in_endgame(&self) -> bool {
        self.pending + self.complete == self.total
    }
}

/// Limited or full information of a torrent's peer session.
#[derive(Debug, Clone)]
pub enum Peers {
...
pub struct PeerSessionsStats {
    ...
    /// Updated when the peer sends us its peer id in the handshake.
...
pub struct ThruputStats {
    ...
    pub payload: Channel,
    /// If a peer has sent us data that we already have, it is recorded here
    /// as waste.
    pub waste: u64,
}
