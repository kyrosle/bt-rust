...
///
/// This is the main entity responsible for the high-level management
/// of a torrent download or upload. It starts and stops connections with
...
pub struct Torrent {
    ...
    ///
    /// This is a separate field as `Instant::now() - start time`
    /// cannot be relied upon due to the fact that it is possible to pause a torrent,
    /// in which case we don't want to record the run time.
    run_duration: Duration,

    /// In the last part of the download the torrent is in what's called
    /// the endgame. This is the stage when all pieces have been picked but
    /// not all have been received.
    /// There is a tendency for a piece to be mostly
    ...
    ///
    /// This is set to some if the configuration is enabled, and set to
    /// none if disabled.
...
impl Torrent {
        ...
        (
            Self {
                ...
                ctx: Arc::new(TorrentContext {
                    ...
                    client_id,
                    cmd_tx: cmd_tx.clone(),
                    piece_picker: Arc::new(
                        RwLock::new(piece_picker),
                    ),
                    downloads: RwLock::new(
                        HashMap::new(),
                    ),
                    alert_tx,
                    disk_tx,
    ...
    pub async fn start(
            ...
            .await
        {
            ...
            // as that is handled inside the function.
            self.ctx
                .alert_tx
    ...
    async fn run(&mut self) -> TorrentResult<()> {
        ...
        // the bind port may have been 0, so we need to get the acutal
        // port in use.
                ...
                Some(cmd) = self.cmd_rx.recv() => {
                    match cmd {
                        ...
                        Command::ReadError { block_info, error } => {
                            ...
                            // TODO: For now we just log for simplicity's sake,
                            // but in the future we'll need error recovery mechanism here.
                            // For instance, it may be that the torrent file got moved
    ...
    ///
    /// This is when we update statistics and report them to the user, when new peer
    /// are connected, and when periodic announces are made.
    async fn tick(
        &mut self,
    ...
    fn connect_peers(&mut self) {
            ...
            .drain(0..connect_count)
        {
            ...
            self.peers.insert(
                addr,
                PeerSessionEntity::start_outbound(
                    session, tx,
                ),
            );
        }
    ...
    /// changed.
    ///
    /// It simply updates the minimum copy fo the peer's state that is kept in
    ...
    /// Does some bookkeeping to mark the piece as finished.
    /// All peer sessions are notified of the newly downloaded piece.
    async fn handle_piece_completion(
        &mut self,
        piece: PieceCompletion,
    ) -> TorrentResult<()> {
        // if this were completed a piece,
        ...
        if piece.is_valid {
            ...
            // In this case we need to enter endgame mode,
            ...
            for peer in self.peers.values() {
                if let Some(tx) = &peer.tx {
                    // this may be after the peer session had already
                    // stopped but before the torrent tick ran and
                    // got a chance to reap the dead session.
                    tx.send(
                        peer::Command::PieceCompletion {
                            index: piece.index,
...
/// A peer in the torrent. Contains additional metadata needed by torrent
/// to manage the peer.
struct PeerSessionEntity {
...
struct TrackerEntry {
    ...
    /// The interval minimum interval at which we can contact tracker.
    /// This is set after the first announce request.
