...
impl FileInfo {
  /// Returns a range that represents the file's first and
  /// one past the last bytes' offsets in the torrent.
  ...
  ///         This may exceed the file length, in which case the returned file
  ...
  pub fn get_slice(
    ...
    FileSlice {
      ...
      len: len
        .min(torrent_end_offset - torrent_offset),
    }
  }
}
...
pub struct StorageInfo {
  ...
  /// file is downloaded, named as the torrent.
  ///
  /// In case of archive downloads, this directory is the download directory joined
  /// by the torrent's name.
  /// This is because in case of a torrent that has multiple top-level entries,
  /// the download files would be scattered across the download directory, which
  /// is an annoyance we want to avoid.
  /// E.g. downloading files into ~/Downloads/<torrent>
...
impl StorageInfo {
  ...
  pub fn new(
    ...
    let last_piece_len = download_len
      - piece_len as u64 * (piece_count - 1) as u64;
  ...
  pub fn files_intersecting_piece(
    ...
    let piece_end =
      piece_offset + self.piece_len(index) as u64;
...
mod tests {
  ...
  #[test]
  fn test_files_interesting_pieces() {
    // single file
    let piece_count = 4;
    let piece_len = 4;
