...
pub struct ThruputCounters {
    ...
    /// Counts the (downloaded) payload bytes that were wasted
    /// (i.e. duplicated blocks that had to be discarded).
    pub waste: Counter,
}
...
/// [`Counter::reset`] which updates the running average and clears the per
/// round counter.
///
/// The tallied throughput rate is the 5 second weighed running average.
